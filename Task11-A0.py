...
TM = TransferMatrix(initStack,[0.01,0.01])
l = plt.plot(wl,_np.mod(_np.angle(TM.trackSingleBeam([-1,0,1],wl,0,False)),2*_const.pi))
plt.legend([-1,0,1])
...
def update(val):
	...
	yval = _np.mod(_np.angle(TM.trackSingleBeam(interfaces,wl,0,False)),2*_const.pi)
